...
module.exports = (function(){
	...
	AttackMode.prototype.attack = function(targetId, collection){
		...
		while(rate--){
			...
			if(type === 'CollateralDamage'){
				collection.getAll().forEach(collateralIterator(attack.enemyId, type, attacks));
			}
			if (collection.isEmpty()){
				break;
			}
	...
	collateralIterator = function(enemyId, type, attacks){
		return function(enemy){
			if(enemy._id !== enemyId){
				attacks.push({ enemyId : enemyId, damage : game.collateralDamage });
