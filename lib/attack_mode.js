...
module.exports = (function(){
	...
	var getAttack, collateralIterator, instances;
	...
	/**
	 * Return a cached list of attack modes.
	 *
	 * @static
	 * @return Object
	 */
	AttackMode.getModes = function(){
		if (!instances){
			instances = {};
			_.each(config, function(mode, type){
				instances[mode.slug] = new AttackMode(type);
			});
		}
		return instances;
	};
			...
			wave.getAll().forEach(function(enemy){
				if(enemy._id !== enemyId && enemy.position === position){
					attacks.push({
						...
						effectiveDamage: Math.min(game.collateralDamage, enemy.hp),
						position : position
					});
